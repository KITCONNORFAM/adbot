...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user.id):
        await update.message.reply_text("⚠️ *This command is only for admins.*", parse_mode="HTML")
        return
...
async def toggle_forward_mode(query, user_id):
    ...
    db.update_user(user_id, use_forward_mode=new_mode)
...
async def toggle_auto_reply(query, user_id):
    ...
    db.update_user(user_id, auto_reply_enabled=new_mode)
...
async def toggle_auto_group_join(query, user_id):
    ...
    db.update_user(user_id, auto_group_join_enabled=new_mode)
...
async def set_target_all_groups(query, user_id):
    db.update_user(user_id, target_mode="all")
...
async def show_selected_groups_menu(query, user_id):
    db.update_user(user_id, target_mode="selected")
...
"""
    ...
    for acc in accounts:
        stats = db.get_account_stats(acc.get('_id'))
...
async def delete_ad_text(query, user_id):
    user = db.get_user(user_id)
    if user and user.get('ad_text'):
        db.update_user(user_id, ad_text=None)
        deleted_text = """
✅ *ᴀᴅ ᴛᴇxᴛ ᴅᴇʟᴇᴛᴇᴅ*
...
async def select_single_account(query, user_id, account_id):
    db.update_user(user_id, selected_single_account=account_id)
...
"""
    ...
    db.update_user(user_id, use_multiple_accounts=True)
...
"""
    ...
    db.update_user(user_id, selected_accounts=selected)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_state == "awaiting_api_id":
        ...
        await update.message.reply_text("⏳ *Verifying 2FA...*", parse_mode="HTML")
        ...
        ad_text = text.strip()
        db.update_user(user_id, ad_text=ad_text)
        del user_states[user_id]
        ...
        try:
            ...
            added = db.add_target_group(user_id, group_id)
            del user_states[user_id]
